...
float alpha_angle = 23.9;	//alpha_angle = 카메라 머리 숙인 각도
float beta_angle = 45.5999; 	//beta_angle = erp 헤딩으로부터 카메라 각도
...
{   
	...
	VideoCapture capLeft("/home/kroad/catkin_ws/src/record_video/stereo_video/kcity_left_0918_1.mp4");
    VideoCapture capRight("/home/kroad/catkin_ws/src/record_video/stereo_video/kcity_right_0918_1.mp4");
    ...
    while (ros::ok()) 
    {
		...
		if((mission_flag == 10)||(mission_flag == 6))
		{
			...
			{	
				...
				double default_array[6] = {1.7, -2.6, 2.4, -3.8, 3.1, -5.0};	//#############################################3좌표계산######################################################################################
	...
	//#############################################################################################################################################
					...
					if(array_count == 20)
					{
						...
						if((abs(pub_array[2]-default_array[2]) > 2.5) || (abs(pub_array[3]-default_array[3]) > 2.5))
						{
							cout << "주차 실패 ㅠㅠ" << endl;
							center_XZ_msg.data.clear();
							for(int i=0; i < 6; i++)

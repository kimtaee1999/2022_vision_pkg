...
Mat StereoVision::find_edge(Mat& frame, int camera) {
	...
	Mat dst2 = Mat::zeros(frame.size(), CV_8UC1 );
	Mat dst3 = Mat::zeros(frame.size(), CV_8UC1 );

	for (int r = 0; r < frame.rows; r++)
	{
        for (int c = center_x; c < frame.cols-2; c++)
		{
			if((gray_image.at<uchar>(r, c+1) + gray_image.at<uchar>(r, c+2) + gray_image.at<uchar>(r, c+3) + gray_image.at<uchar>(r, c+4) + gray_image.at<uchar>(r, c+5))-5*gray_image.at<uchar>(r, c) > 60) // 60
			{
	...
	for (int r = 0; r < frame.rows; r++)
	{
		...
		{
			if(5*gray_image.at<uchar>(r, c) - (gray_image.at<uchar>(r, c+1) + gray_image.at<uchar>(r, c+2) + gray_image.at<uchar>(r, c+3) + gray_image.at<uchar>(r, c+4) + gray_image.at<uchar>(r, c+5)) > 60) // 60
			{
				dst.at<uchar>(r, c-1) = 255;
				dst.at<uchar>(r, c) = 255;
				dst.at<uchar>(r, c+1) = 255;
	...
	for (int c = 0; c < frame.cols; c++) 
	{
		...
		{
			if((gray_image.at<uchar>(r+1, c) + gray_image.at<uchar>(r+2, c) + gray_image.at<uchar>(r+3, c) + gray_image.at<uchar>(r+4, c) + gray_image.at<uchar>(r+5, c)) - 5*gray_image.at<uchar>(r, c) > 60) // 60
			{
				dst.at<uchar>(r+5, c-20) = 255;
				dst.at<uchar>(r+6, c-21) = 255;
				dst.at<uchar>(r+7, c-22) = 255;
				dst.at<uchar>(r+8, c-23) = 255;
				dst.at<uchar>(r+9, c-24) = 255;
			}
      }
    }
	...
	for (int c = 0; c < frame.cols; c++) 
	{
		...
		{
			if(5*gray_image.at<uchar>(r, c) - (gray_image.at<uchar>(r+1, c) + gray_image.at<uchar>(r+2, c) + gray_image.at<uchar>(r+3, c) + gray_image.at<uchar>(r+4, c) + gray_image.at<uchar>(r+5, c)) > 60) // 60
			{
				dst.at<uchar>(r-10, c) = 255;
				dst.at<uchar>(r-11, c) = 255;
				dst.at<uchar>(r-12, c) = 255;
				dst.at<uchar>(r-13, c) = 255;
				dst.at<uchar>(r-14, c) = 255;
			}
        }
    }
//----------------------------------------------------------------------
	...
	// 		{
	// 			// dst.at<uchar>(r, c-2) = 255;
	// 			dst2.at<uchar>(r, c-1) = 255;
	// 			dst2.at<uchar>(r, c) = 255;
	// 			dst2.at<uchar>(r, c+1) = 255;
	// 			// dst.at<uchar>(r, c+2) = 255;
	// 		}
    //     }
    ...
    //     for (int r = 0; r < center_y; r++)
	// 	{
	// 		if((5*gray_image.at<uchar>(r, c) - gray_image.at<uchar>(r+1, c) - gray_image.at<uchar>(r+2, c) - gray_image.at<uchar>(r+3, c) - gray_image.at<uchar>(r+4, c) - gray_image.at<uchar>(r+5, c)) > 150) // 60
	// 		{
	// 			// dst.at<uchar>(r, c-2) = 255;
	// 			// dst2.at<uchar>(r-1, c) = 255;
	// 			dst2.at<uchar>(r, c) = 255;
	// 			// dst2.at<uchar>(r+1, c) = 255;
	// 			// dst.at<uchar>(r, c+2) = 255;
	// 		}
	...
	// for (int c = 0; c < frame.cols; c++)
	// {
    //     for (int r = center_y; r < frame.rows-2; r++)
	// 	{
	// 		// if((gray_image.at<uchar>(r+31, c) + gray_image.at<uchar>(r+32, c) + gray_image.at<uchar>(r+33, c) + gray_image.at<uchar>(r+34, c) + gray_image.at<uchar>(r+35, c) - 5*gray_image.at<uchar>(r, c)) > 60)
	// 		// {
	// 		// 	dst2.at<uchar>(r-1, c) = 0;
	// 		// 	dst2.at<uchar>(r, c) = 0;
	// 		// 	dst2.at<uchar>(r+1, c) = 0;
	// 		// }
	// 		if((gray_image.at<uchar>(r+1, c) + gray_image.at<uchar>(r+2, c) + gray_image.at<uchar>(r+3, c) + gray_image.at<uchar>(r+4, c) + gray_image.at<uchar>(r+5, c) - 5*gray_image.at<uchar>(r, c)) > 150) // 60
	// 		{
	// 			// dst.at<uchar>(r, c-2) = 255;
	// 			// dst2.at<uchar>(r-1, c) = 255;
	// 			dst2.at<uchar>(r, c) = 255;
    ...
    //     for (int c = center_x; c < frame.cols-2; c++)
	// 	{
	// 		if((5*gray_image.at<uchar>(r, c) - gray_image.at<uchar>(r, c+1) - gray_image.at<uchar>(r, c+2) - gray_image.at<uchar>(r, c+3) - gray_image.at<uchar>(r, c+4) - gray_image.at<uchar>(r, c+5)) > 40) // 60
	// 		{
	// 			dst.at<uchar>(r-3, c-2) = 255;
	// 			dst.at<uchar>(r-2, c-2) = 255;
	// 			dst3.at<uchar>(r-1, c) = 255;
	// 			dst3.at<uchar>(r, c-1) = 255;
	// 			dst3.at<uchar>(r, c) = 255;
	// 			dst3.at<uchar>(r, c+1) = 255;
	// 			dst3.at<uchar>(r+1, c) = 255;
	// 			dst.at<uchar>(r+2, c) = 255;
	// 			dst.at<uchar>(r+3, c) = 255;
	// 		}
    //     }
    // }
